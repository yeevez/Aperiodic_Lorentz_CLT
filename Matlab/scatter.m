function [paths,flights]=scatter(bounces,trials,step,m,radius,outdim)
  ...
  [sp,~,~]=scatterer_positions(r,window,200,origin);
      ...
      parfor i=1:trials
        ...
        while bounce<=bounces
          ...
          if length(h)>0
            ...
            [~,s]=min(rsp(1,h));
            %position of reflection
            b=rsp(1,h(s))-sqrt(radius^2-(rp(2)-rsp(2,h(s)))^2); %todo: generalize
            %unrotated position of reflection
            ...
            position = position_new
            %angle of reflection
            angle=mod(2*atan2(rsp(2,h(s))-rp(2),rsp(1,h(s))-b)+pi+angle,2*pi); %todo: generalize
            path(:,bounce)=position;
            %fprintf('Bounced at (%.2f,%.2f).\n',position(1),position(2));
          else
            %passes through without reflection, computes convex hull of center of scatterers
            ...
            %computes intersection of scattering direction with convex hull, taking right-most intersection to be exit point
            m=h(:,2:end)-h(:,1:(end-1)); %todo: generalize
            m=m(2,:)./m(1,:); %todo: generalize
            x=(rp(2)-h(2,1:(end-1))+m.*h(1,1:(end-1)))./m; %todo: generalize
            ...
            c=round(rinv*[[cos(angle),-sin(angle);sin(angle),cos(angle)]*[max(x(xi))-radius;rp(2)];0]); %todo: generalize
            %computes new grid centered at exit point
            [sp,~,~]=scatterer_positions(r,window,grid,c);
          end
        end
